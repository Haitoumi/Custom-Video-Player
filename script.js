...
container.addEventListener("mousemove", () => {
  container.classList.add("show-controls"); //add show controls class on mousemouve
  clearTimeout(timer); //clear timer
  hideControls(); //calling hideControls
});

const formatTime = (time) => {
  // gettng seconds, minutes, hours
...
mainVideo.addEventListener("loadeddata", (e) => {
  videoDuration.innerText = formatTime(e.target.duration); //passing video duration as video duration innerText
});
...
const draggableProgressBar = (e) => {
  let timelineWidth = videoTimeline.clientWidth; //getting video time line width
...
videoTimeline.addEventListener("mousedown", () => {
  //calling draggable function on mousemove event
...
volumeBtn.addEventListener("click", () => {
  if (!volumeBtn.classList.contains("fa-volume-high")) {
    //If volume icon is not volume high icon
...
volumeSlider.addEventListener("input", (e) => {
  mainVideo.volume = e.target.value; //passing slider value as a video volume
  if (e.target.value == 0) {
